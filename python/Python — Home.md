...
```
Langage   : Bases → OOP → Générateurs → Typing → Décorateurs → asyncio → Pydantic → FastAPI
                                                                                    → SQLAlchemy

Données   : Bases → NumPy → Pandas → Matplotlib / Seaborn

IA        : NumPy → Pandas → ML (scikit-learn)
```

## Prérequis & suite

- [[Home]] ← retour accueil
- [[Math — Home]] ← algèbre linéaire (prérequis IA)
